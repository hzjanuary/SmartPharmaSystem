...
- Biến chính:
	...
	- `SESSION_SECRET`
	- `FRONTEND_ORIGIN`
	- `TELEGRAM_BOT_TOKEN`, `ADMIN_CHAT_ID` (dùng cho thông báo đăng nhập)
...
- Đọc system log tổng quát qua `get_system_logs`
- Đọc login/action logs theo từ khóa qua `get_login_action_logs`
- Đọc lịch sử nhập kho chi tiết qua `get_import_history`
- Tra cứu SKU qua `search_sku`
- Đếm loại hàng qua `count_product_types`
- Liệt kê danh sách sản phẩm qua `list_products`
- Liệt kê sản phẩm đã nhập theo tháng qua `list_imported_products_by_month`
...
- "Hiện tại trong database có bao nhiêu sản phẩm và cụ thể là những sản phẩm nào?"
- "Tháng này đã nhập những sản phẩm nào?"
- "Cho tôi login logs 20 bản ghi gần nhất"
...
## 10. Tài liệu giao việc 2 team
- File giao việc mới nhất cho Backend + Frontend: `taskmar21st.md`
- Mục đích: thống nhất yêu cầu hiển thị user đăng nhập, lưu login/action logs, và chuẩn dữ liệu import history cho SPS Agent.
